...
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
// NOTE: Agar aapne createFakeJWT.ts file ko delete kar diya hai, toh yeh import hata den.
// Hum isko directly yahan define kar rahe hain.
// import { createFakeJWT } from "./jwttoken"; 
...
// =========================================================
//              BEGINNER-FRIENDLY JWT LOGIC
// =========================================================

// JWT ka Base64 URL-safe encoding function
...
/**
 * [STEP 1: JWT BANANA] - Ek Fake JWT banata hai
 * Logic: Header + Payload + Secret ka Fake Signature
 */
function createFakeJWT(payload: object, secret: string): string {
  const header = {
    alg: "HS256",
    typ: "JWT",
  };
  ...
  // Fake Signature: Secret ka Base64 karke, uske shuruaati 16 letters le liye
  const signature = btoa(secret).slice(0, 16); 
...
const JWT_SECRET = "hardcodedSecret"; // For demo only (Yeh Secret key hai)
...
function Login() {
  ...
  useEffect(() => {
    // Shuru mein check karte hain agar user pehle se logged in hai ya nahi
    const token = localStorage.getItem("authToken");
    
    // NOTE: Yahan hum verifyFakeJWT use karke token ki validity check kar sakte hain, 
    // lekin abhi hum sirf 'LoggedIn' flag aur token ki existence check kar rahe hain, 
    // taaki code simple rahe.
    if (localStorage.getItem("LoggedIn") === "true" && token) {
        // Agar logged in hai, toh seedha table page par bhej do
        navigate("/table");
    }

    // Default user ko load karna
    const storedUsers = localStorage.getItem("users");
    if (storedUsers) {
      setUsers(JSON.parse(storedUsers));
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      // JWT Step 2: Token banana (Humara simple function use karke)
      const token = createFakeJWT(payload, JWT_SECRET);

      // JWT Step 3: Token ko save karna aur redirect karna
      localStorage.setItem("authToken", token);
      localStorage.setItem("LoggedIn", "true");
      navigate("/table");
      ...
      // JWT Step 1: Payload banana (naye user ke liye)
      const payload = {
        userId: newUser.id,
        name: newUser.name,
        email: newUser.email,
  ...
  // =========================================================
  //              UI (DESIGN) - Jaisa aapne maanga tha
  // =========================================================
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 relative overflow-hidden">
      {/* Animated Background Particles */}
